...
class WIREGUARD(asyncio.DatagramProtocol):
    ...
    def datagram_received(self, data, addr):
        ...
        if cmd == 1 and len(data) == 148:
            def HASH(x): return hashlib.blake2s(x).digest()
            def MAC(key, x): return hashlib.blake2s(
                x, key=key, digest_size=16).digest()

            def HMAC(key, x): return hmac.digest(key, x, hashlib.blake2s)
            ...
            hash0 = HASH(HASH(HASH(chaining_key + b"WireGuard v1 zx2c4 Jason@zx2c4.com") +
                         self.public_key) + unencrypted_ephemeral)
            chaining_key = HMAC(
                HMAC(chaining_key, unencrypted_ephemeral), b"\x01")
            temp = HMAC(chaining_key, crypto.X25519(
                self.private_key, unencrypted_ephemeral))
            ...
            temp = HMAC(chaining_key, crypto.X25519(
                self.private_key, static_public))
            chaining_key = HMAC(temp, b"\x01")
            ...
            chaining_key = HMAC(HMAC(HMAC(HMAC(HMAC(HMAC(chaining_key, ephemeral_public), b"\x01"), crypto.X25519(
                ephemeral_private, unencrypted_ephemeral)), b"\x01"), crypto.X25519(ephemeral_private, static_public)), b"\x01")
            ...
            encrypted_nothing = crypto.aead_chacha20poly1305_encrypt(
                key, 0, b"", hash0)
            # hash0 = HASH(hash0 + encrypted_nothing)
            msg = struct.pack('<III32s16s', 2, index, sender_index,
                              ephemeral_public, encrypted_nothing)
            msg = msg + MAC(HASH(b"mac1----" + static_public),
                            msg) + b'\x00'*16
            ...
            packet = crypto.aead_chacha20poly1305_decrypt(
                receiving_key, counter, data[16:], b'')

            def reply(data):
                counter = next(self.index_generators[index])
                data = data + b'\x00'*((-len(data)) % 16)
                msg = crypto.aead_chacha20poly1305_encrypt(
                    sending_key, counter, data, b'')
...
def main():
    ...
    parser.add_argument('-wg', dest='wireguard', default=None, type=int,
                        help='start a wireguard vpn with port number (default: off)')
    ...
    if args.wireguard:
        transport3, _ = loop.run_until_complete(loop.create_datagram_endpoint(
            lambda: WIREGUARD(args), ('0.0.0.0', args.wireguard)))
